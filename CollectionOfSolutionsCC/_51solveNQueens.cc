...
vector<vector<string>> Leetcode1_100::Solution::solveNQueens(int n)
{
    // 非常经典的回溯类问题
    ...
    function<void(vector<vector<string>>&, vector<string>&, int)> backtrack = [&](vector<vector<string>>& res, vector<string>& cur, int x)
    {
        if(x == n)
        {
            res.push_back(cur);
